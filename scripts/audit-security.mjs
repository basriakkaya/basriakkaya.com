...
for (const route of ['index.html', path.join('en', 'index.html'), path.join('ben-kimim', 'index.html'), path.join('en', 'about', 'index.html')]) {
  ...
  for (const logo of ['university-of-oslo.svg', 'nasa-logo.svg', 'dhs-logo.svg', 'university-of-twente.svg', 'arcelik.png', 'goce-delcev-university.png', 'rahim-usta-anatolian-high-school.png', 'komsukomsu-logo.png']) if (!html.includes(`/images/recognition/${logo}`)) failures.push(`${route} HOF logosu eksik: ${logo}`);
