...
for (const route of ['index.html', path.join('en', 'index.html'), path.join('ben-kimim', 'index.html'), path.join('en', 'about', 'index.html')]) {
  ...
  for (const value of ['ASUS PSIRT — 2026 Hall of Fame', 'Basri Akkaya (realkage)', 'asus.com/security-advisory/#hof', 'A02:2025 Security Misconfiguration']) if (!html.includes(value)) failures.push(`${route} ASUS HOF içeriği eksik: ${value}`);
  for (const value of ['University of Oslo / UiO-CERT', 'HOF · LOR', 'university-of-oslo-letter-of-recognition.png', route.startsWith('en') ? 'Information Disclosure' : 'Bilgi İfşası']) if (!html.includes(value)) failures.push(`${route} University of Oslo HOF/LOR içeriği eksik: ${value}`);
