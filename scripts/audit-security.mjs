...
for (const route of ['index.html', path.join('en', 'index.html'), path.join('ben-kimim', 'index.html'), path.join('en', 'about', 'index.html')]) {
  ...
  for (const logo of ['nasa-logo.svg', 'dhs-logo.svg', 'university-of-twente.svg', 'arcelik.png', 'goce-delcev-university.png', 'rahim-usta-anatolian-high-school.png']) if (!html.includes(`/images/recognition/${logo}`)) failures.push(`${route} HOF logosu eksik: ${logo}`);
  for (const value of ['U.S. Department of Homeland Security VDP', 'Sensitive Data Exposure', 'Disclosure of Secrets', 'dhs-fema-bugcrowd-evidence.png']) if (!html.includes(value)) failures.push(`${route} DHS HOF içeriği eksik: ${value}`);
}
