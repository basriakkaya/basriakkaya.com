...
export const getPostsByCategory = async (category: string, locale: Locale = 'tr') => (await getPublishedPosts(locale)).filter((post) => post.data.category === category);
export const getPostsBySeries = async (series: string, locale: Locale = 'tr') => (await getPublishedPosts(locale)).filter((post) => post.data.series === series).sort((a, b) => (a.data.seriesOrder ?? 0) - (b.data.seriesOrder ?? 0));
...
export async function getSeriesContext(post: BlogPost) {
  ...
  const posts = await getPostsBySeries(post.data.series, post.data.lang);
...
export async function getRelatedPosts(post: BlogPost, excluded: BlogPost[] = []) {
  ...
  return (await getPublishedPosts(post.data.lang))
...
export async function getTranslation(post: BlogPost, locale: Locale): Promise<BlogPost | undefined> {
  if (!post.data.translationKey) return undefined;
  return (await getPublishedPosts(locale)).find(({ data }) => data.translationKey === post.data.translationKey);
}
